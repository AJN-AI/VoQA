    ...
    def __getitem__(self, index):
        ...
        if self.task == "scienceqa":
            ...
            if self.original_benchmark:
                ...
                if self.model_name in ['llava-v1.5-7b-QRA']: # models which <image> token is not needed.
                    qs = qs.replace('<image>\n', '')
            else:
                if self.model_name == "llava-v1.5-7b" and getattr(self.model_config, 'mm_use_im_start_end', False):
                    qs = DEFAULT_IM_START_TOKEN + DEFAULT_IMAGE_TOKEN + DEFAULT_IM_END_TOKEN + '\n' + self.qs
                elif self.model_name in ['llava-v1.5-7b-Baseline', 'llava-v1.5-7b-QA', 'llava-v1.5-7b-QRA', \
                                        'TinyLLaVA-Qwen2-0.5B-SigLIP-QA', 'TinyLLaVA-Qwen2-0.5B-SigLIP-QRA', \
                                        'TinyLLaVA-Qwen2.5-3B-SigLIP-QRA', 'TinyLLaVA-Qwen2-0.5B-SigLIP-QRA-HELPER', \
                                        'TinyLLaVA-Qwen2-0.5B-SigLIP-QRA-CAT'] # models which <image> token is not needed.
                    qs = '\n' + self.qs
                else:
